...
class Reflector extends Reflectable {
  const Reflector() : super(libraryCapability, const TopLevelInvokeCapability(r'^myFunction$'), declarationsCapability);
}

class Reflector2 extends Reflectable {
  const Reflector2()
      : super(reflectedTypeCapability, const TopLevelInvokeMetaCapability(Test), declarationsCapability,
...
final Matcher throwsNoCapability = throwsA(const TypeMatcher<NoSuchCapabilityError>());
...
void main() {
  ...
  test('invoke function, getter', () {
    var libraryMirror = reflector.findLibrary('test_reflectable.test.libraries_test');
    expect(libraryMirror.simpleName, 'test_reflectable.test.libraries_test');
    expect(libraryMirror.invoke('myFunction', []), 'hello');
    expect(Function.apply(libraryMirror.invokeGetter('myFunction') as Function, []), 'hello');
    expect(() => libraryMirror.invokeGetter('getter'), throwsReflectableNoMethod);
    var myFunctionMirror = libraryMirror.declarations['myFunction'] as MethodMirror;
  ...
  test('No libraryCapability', () {
    ...
    expect(() => reflector3.findLibrary('test_reflectable.test.libraries_test').owner, throwsNoCapability);
  });
}
